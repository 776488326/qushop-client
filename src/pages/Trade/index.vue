...
<script>
  export default {
    ...
    methods:{
      async getTradeInfo(){
        try {
          const result = await this.$store.dispatch('getTradeInfo')
          alert(result)
        } catch (error) {
          alert(error.message)
        }
      ...
      async getAddressInfo(){
        try {
          const result = await this.$store.dispatch('getAddressInfo')
          alert(result)
        } catch (error) {
          alert(error.message)
        }
      ...
      submitOrder(){
        let tradeNo = this.defaultAddress.consignee;
        let tradeInfo = {
          consignee: this.defaultAddress.consignee,
          consigneeTel: this.defaultAddress.phoneNum,
          deliverAddress:this.defaultAddress.fullAddress,
          paymentWay:'ONLINE',
          orderComment:this.message,
          orderDetailList:this.detailArrayList,
        };
        const result = this.$API.reqSubmitOrder(tradeNo,tradeInfo);
        if(result.code===200)
        {
          alert('创建订单成功，准备去往支付页面！')
          this.$router.push('/pay?prderNum='+result.data)
        }
      }
    }
  }
