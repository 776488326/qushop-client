      ...
      <div class="goods">
        <div class="left-good">
          ...
          <div class="right-info">
            ...
            <p class="attr">颜色：{{color}} 版本：{{version}} 数量：{{skuNum}}</p>
          </div>
        </div>
...
<script>
  export default {
    ...
    beforeMount(){
      ...
      const display = this.skuinfo.skuSaleAttrValueList.filter(val=>val.isChecked==="1")
      display.forEach(val => {
          if(val.saleAttrName==='颜色')
          {
            this.color = val.saleAttrValueName
          }
          else{
            this.version = val.saleAttrValueName
          }
        })
    }
  }
